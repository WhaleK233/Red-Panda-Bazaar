...
| 方案 | 贷款金额 | 日利率 | 说明 |
|------|---------|--------|------|
| 方案A | P × 50% | 0.08% | 小额低息，适合短期周转 |
| 方案B | P × 100% | 0.12% | 中等额度，平衡型 |
| 方案C | P × 150% | 0.18% | 大额高息，适合大额需求 |
...
显示总信用额度、已用额度、剩余额度。三种方案各占一块面板，显示该方案可贷金额和日利率，直接选择方案即按该方案上限全额放贷，不提供自定义金额输入。

下方列出当前所有未还贷款，每条显示方案类型、本金、利息和[还款]按钮。

- **申请贷款：** 选择方案 A/B/C → 校验剩余额度 → 全额放贷 → 创建 `LoanAccount`
- **还款：** 选中某笔贷款 → 从玩家扣 `Principal + InterestAccrued` → 标记已还清
