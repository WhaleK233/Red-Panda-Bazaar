...
namespace Red_Panda_Bazaar_Code.Features.TargetMiniGame;

public class TargetMiniGame : IMinigame
{
    public int score;
    ...
    private static TargetMiniGame? _currentInstance;
    ...
    public string minigameId() => nameof(TargetMiniGame);
    ...
    {
        Game1.currentMinigame = (IMinigame)new TargetMiniGame();
    ...
    public class Target
    {
        ...
        public void shatter(GameLocation location, Projectile stone)
        {
            ...
            if (_currentInstance != null) _currentInstance.score += number;
            if (!(stone is BasicProjectile basicProjectile) || basicProjectile.damageToFarmer.Value <= 0)
                return;
            if (_currentInstance != null) ++_currentInstance.successShots;
