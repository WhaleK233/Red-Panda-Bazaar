...
public static class FishingMiniGamePatch
{
    private const string NAME = nameof(FishingMiniGamePatch);
    ...
    public static void ApplyPatch(Harmony harmony)
    {
        ...
        harmony.Patch(
            ...
            prefix: new HarmonyMethod(typeof(FishingMiniGamePatch), nameof(Prefix_Event_caughtFish))
        );

        Tools.LogPatch(NAME, "Event.perfectFishing()", PatchType.Prefix);
        harmony.Patch(
            original: AccessTools.Method(typeof(Event), "perfectFishing"),
            prefix: new HarmonyMethod(typeof(FishingMiniGamePatch), nameof(Prefix_Event_perfectFishing))
        );
    }
    ...
    private static bool Prefix_Event_caughtFish(Event __instance, int size)
    {
        try
        {
            if (Game1.currentMinigame is Features.FishingMiniGame.FishingMiniGame currentMinigame &&
